...
pub struct MediumEncryptor<'a, S: 'a + Storage> {
    pub storage: &'a mut S,
    pub buffer: Vec<u8>,
    original_chunks: Option<Vec<ChunkDetails>>,
}

impl<'a, S: Storage> MediumEncryptor<'a, S> {
    ...
    pub fn new(storage: &'a mut S,
               ...
               -> Result<MediumEncryptor<'a, S>, SelfEncryptionError<S::Error>> {
...
impl<'a, S: Storage> From<SmallEncryptor<'a, S>>
        for MediumEncryptor<'a, S> {
    fn from(small_encryptor: SmallEncryptor<'a, S>) -> MediumEncryptor<'a, S> {
        MediumEncryptor {
