    ...
    S: Storage + 'static + Send + Sync,
{
    ...
    pub async fn new(storage: S, data: Vec<u8>) -> Result<SmallEncryptor<S>, SelfEncryptionError> {
...
mod tests {
    ...
    async fn basic_write_and_close(data: &[u8]) -> Result<(), SelfEncryptionError> {
        let (data_map, storage) = {
            ...
            let mut encryptor = SmallEncryptor::new(storage, vec![]).await?;
    ...
    async fn multiple_writes_then_close<T: Rng>(
        ...
        for data in data_pieces {
            let (data_map, storage) = {
                let storage = SimpleStorage::new();
                let mut encryptor = SmallEncryptor::new(storage, existing_data.clone()).await?;
                encryptor = encryptor.write(data).await?;
