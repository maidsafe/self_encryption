...
impl<'a, E: StorageError, S: Storage<E>> Encryptor<'a, E, S> {
    ...
    /// Buffers some or all of `data` and stores any chunks which cannot be modified by subsequent
    /// `write()` calls.  The internal buffers can only be flushed by calling `close()`.
