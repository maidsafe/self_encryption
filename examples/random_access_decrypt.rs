...
fn main() -> Result<()> {
    ...
    println!("Created streaming decrypt instance with batch size 5\n");
    ...
    for (i, (start, len)) in positions.iter().enumerate() {
        ...
        println!("   Range {}: {} bytes at position {} ✓", i + 1, range_data.len(), start);
    ...
    println!("   Requesting range beyond file: {} bytes returned", beyond_range.len());
    ...
    println!("   Requesting 200 bytes with only 100 available: {} bytes returned", partial_exceed.len());
